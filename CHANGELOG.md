...
- **Upstream Sep-12 window (2026-09-05..2026-09-12, branch `feat/upstream-sep12-tier1`)**: FRED keeps `response` on redacted errors + scrubs 400 body (#1324); StockTwits 5MiB feed cap (#1328); Alpha Vantage global-news optionals resolve from config (#1329); `curr_date` required on fundamental statement tools (#1331); `propagate()` rejects future trade dates (#1319); conda no-sudo docs (#1312); `tradingagents/signals/` provider abstraction + mock (#1318); opt-in Keenable news vendor, excluded from defaults (#1308); CLI decision-log lifecycle `prepare_memory_context`/`record_decision` (#1332); broker-neutral first-class `PortfolioContext` on the canonical path, IBKR tool kept distinct (#1304). Verified SKIP: checkpointer SQL already static (#1307), OHLCV cache already fixed-name (#1330). Rejected: China A-share mega-break (#1303), Vietnam pack (#1309). (`tradingagents/dataflows/fred.py`, `stocktwits.py`, `alpha_vantage_news.py`, `keenable_news.py`, `agents/utils/fundamental_data_tools.py`, `graph/trading_graph.py`, `cli/main.py`, `signals/`, `agents/schemas.py`, `docs/UPSTREAM_PR_INTEGRATION_PLAN_2026-09-12.md`)
...
- **Upstream Aug 23 window — full integration (37 PRs, 2026-07-23..2026-08-23)**: Binance native crypto OHLCV vendor (`binance` for `core_stock_apis`, CMC20 whitelist 18, paginated 1000-candle, `BINANCE_BASE_URL` for US) (#1244); risk-debator fabrication guard extended to Aggressive/Conservative/Neutral (`_format_risk_opponent` placeholder) (#1233); social sentiment window filtering with `_window_epochs/_in_window` and `through {end_date}` placeholder — prevents look-ahead leakage (#1232); market-session-aware news windows (`news_window` `market_session` via `exchange-calendars`, NYSE/NASDAQ, DST/holiday/early-close aware, opt-in) (#1235); MiniMax M3 configurable thinking (`thinking_modes` `adaptive`/`disabled`, reasoning_split via extra_body, cap table split M2/M3) (#1229); crypto on-chain signals (Fear&Greed + mempool hashrate, keyless, plus CryptoCurrency/Bitcoin/ethereum subreddits gated on `asset_type==crypto`) (#1209); backtest harness `evaluation/backtest.py` + `scripts/backtest.py` (ticker×date grid, 5-tier, forward alpha vs benchmark) (#1234); AnySearch/Schwab/Newsflash/Firecrawl/Eastmoney stub vendors + Atlas Cloud provider (all opt-in, graceful fallback, `VendorNotConfiguredError`) (#1207/#1221/#1164/#1183/#1181); API-key scrubbing (`_scrub_api_key`, PR #1238) and `.env.example` vendor keys (#1184). Rejected: Alpaca live automation (#1237, violates `AGENTS.md` advisory-only), BIST KAP (#1246, narrow TR market, closed), AgentRouter (#1230, provider churn). Deferred: pydantic-ai/mem0/graphiti proxy (#1202, leaks `output/`), type-annotation base refactor (#1185). (`tradingagents/dataflows/{binance,crypto_signals,news_window,firecrawl_news,newsflash,schwab,anysearch,eastmoney_data}.py`, `tradingagents/agents/risk_mgmt/*.py`, `tradingagents/dataflows/{reddit,stocktwits}.py`, `tradingagents/agents/analysts/sentiment_analyst.py`, `tradingagents/llm_clients/capabilities.py`, `tradingagents/evaluation/backtest.py`, `tradingagents/default_config.py`, `.env.example`)
...
- **Read-only IBKR portfolio context for the Trader**: new `dataflows/ibkr.py::get_portfolio_context()` returns cash, net liquidation (equity), buying power, and open positions from the connected IBKR account as Markdown, using the same `ib_insync.accountSummary()`/`.positions()` calls `ops/broker/ibkr_client.py` already proves safe for the live-trading daemon — but as a separate, strictly read-only function with no order-placement path reachable from it, and one that never reads an account ID/number out of the summary rows (`persistence/`/`evidence/` don't serialize tool output at all, so there's nothing to scrub or leak either way). Wired as a new `trader_get_ibkr_portfolio` tool, gated off-by-default behind a new `ibkr_portfolio_context_enabled` config key (`TRADINGAGENTS_IBKR_PORTFOLIO_CONTEXT_ENABLED` env override) since — unlike the trader's other three tools — it requires a running TWS/IB Gateway most users won't have. `_ctx()` (shared by every `dataflows/ibkr.py` function) now also sets `ib.RequestTimeout` so a stalled TWS/Gateway response can't hang a call indefinitely, and picks a random `clientId` per connection unless `ibkr_client_id` is explicitly configured, avoiding a same-fixed-id collision under concurrent tool/graph calls. Rescopes upstream #1125 down from its originally-planned "new design doc" workstream once `ops/broker/ibkr.py` showed the connection pattern is small and already proven; also resolves #1117's requested position-exit engine as **already built** (`ops/exits/engine.py`'s `rank_decay`/`trend_break`/`max_hold` rules), moving it from a planned design-doc workstream to Tier 3/Skip in `docs/UPSTREAM_PR_INTEGRATION_PLAN.md`. Also fixes a whitespace-padded vendor config (e.g. `" yfinance "`) silently skipping East Money prioritization for A-share news in `_resolve_vendor_chain`. (`tradingagents/dataflows/ibkr.py`, `tradingagents/dataflows/interface.py`, `tradingagents/agents/trader/trader_tools.py`, `tradingagents/graph/setup.py`, `tradingagents/default_config.py`, `tradingagents/config_schema.py`, `docs/CONFIG_REFERENCE.md`, `docs/UPSTREAM_PR_INTEGRATION_PLAN.md`, `tests/test_ibkr_portfolio_context.py`, `tests/test_eastmoney_news.py`)

- **Trade-horizon context wired into the trader's prompt**: new `trader/trader_system.v5.txt` appends `${horizon_instruction}`, rendered from `agent_utils.get_horizon_instruction()` — a function that already existed in this fork (per-bucket guidance for `1_day`/`1_week`/`1_month`/`6_months`/`1_year`/`5_years_plus`/`medium_term`, driven by the existing `investment_horizon` config key) but was never actually called from any agent. Rebuilds upstream #1147's underlying idea (inject trade-horizon context into agent prompts) against this fork's existing, more granular horizon system rather than the PR's flat `trade_horizon_days`/`entry_price`/`stop_loss_pct` numeric injection, and drops the PR's unrelated Reddit/StockTwits dataflow changes entirely. Shipped available, not default (`trader/trader_system` stays on `v3`). (`tradingagents/agents/trader/trader.py`, `tradingagents/prompts/trader/trader_system.v5.txt`, `tests/test_structured_agents.py`, `tests/test_agent_utils_extended.py`, `tests/test_prompt_registry_cli.py`)

- **Futures asset type**: `AssetType.FUTURES` joins `STOCK`/`CRYPTO` in `cli/models.py`, using the same extension points those two already established rather than a separate module. `detect_asset_type()` classifies a symbol as futures on the canonical (post-`normalize_symbol`) ticker ending in `=F` — covering both a directly-typed Yahoo futures symbol (`ES=F`, `CL=F`) and a broker-style commodity/forex alias that already resolves to one (`GOLD`/`XAUUSD` → `GC=F`, `WTI` → `CL=F`). `filter_analysts_for_asset_type()` now drops the Fundamentals Analyst for futures too (no company financials, same reasoning as crypto). Fixed a pre-existing test (`test_cli_symbol_handling.py::test_detect_asset_type[GC=F]`) that had asserted the old, incorrect `STOCK` classification for `GC=F`. Ports upstream #1155. (`cli/models.py`, `cli/utils.py`, `cli/main.py`, `tests/test_crypto_asset_mode.py`, `tests/test_cli_symbol_handling.py`)

- **Research Manager / Portfolio Manager prompt hardening against tool hallucination**: new `managers/research_manager.v3.txt` and `managers/portfolio_manager.v4.txt` add "Do not call any external tools or search the web — all necessary information is already provided below." Neither agent binds any tools in this fork, so a hallucinated tool-call attempt can't actually invoke anything — but the explicit instruction still reduces the chance of one showing up in the output text. Shipped **available, not default** — `research_manager.v1`/`portfolio_manager.v2` stay the active defaults, byte-identical to what's already deployed (verified by a hash-pinned regression test), per this fork's prompt-registry immutability convention. Ports the prompt-hardening half of upstream #1131 only — the trader's equivalent instruction was deliberately **not** ported, since our trader (unlike upstream's) has real, legitimate tools bound (`trader_get_current_price`/`_options_overview`/`_news_summary`) and telling it to never call tools would break that; the PR's DuckDuckGo-fallback module half was also not ported, since upstream itself rejected that half for external-dependency/non-determinism reasons and this fork already has `dataflows/searxng.py` as its sanctioned web-search fallback. (`tradingagents/prompts/managers/research_manager.v3.txt`, `portfolio_manager.v4.txt`, `tests/test_prompt_hardening_v3.py`)

- **Env-var overrides to skip the report save/display prompts**: `TRADINGAGENTS_SAVE_REPORT` and `TRADINGAGENTS_DISPLAY_REPORT` join the CLI's `TRADINGAGENTS_*` non-interactive override convention, wiring up `cli/main.py`'s existing `should_save_report()`/`should_display_report()`/`resolve_report_save_path()` helpers — which were already fully unit-tested but never actually called from `run_analysis()`. A new `TRADINGAGENTS_SAVE_REPORT_PATH` overrides the save path when saving is env-driven (falls back to the same timestamped default path the interactive prompt uses). Also added the missing `@pytest.mark.unit` marker to `tests/test_cli_report_saving.py`'s existing tests, which were silently excluded from `pytest -m unit` runs. Ports upstream #1135. (`cli/main.py`, `tests/test_cli_run_analysis.py`, `tests/test_cli_report_saving.py`)
...
- **Refreshed upstream PR integration plan** (`docs/UPSTREAM_PR_INTEGRATION_PLAN.md`): re-triaged the 39 PRs opened against `TauricResearch/TradingAgents` between 2026-07-07 and 2026-07-28, replacing the 2026-07-25 revision. Records the 17 items from the previous window that have since landed here, and cuts the outstanding work to three: port upstream #1179's run manifest onto our existing `audit/schemas.py` canonical-JSON hashing and `reports/exporter.py` (rather than upstream's standalone module, which duplicates hashing we already have and targets a `tradingagents/reporting.py` that doesn't exist in this fork); harden `dashboard/app.py`, which was confirmed to bind `0.0.0.0:8050` with no auth, CSP, or host allowlist; and close the now-obsolete fork PR #22, whose routers landed via #26/#28. Two verdicts changed on re-check — #1120/#1137 (Codex/ChatGPT sign-in) drops to Skip because `llm_clients/openai_oauth_client.py` is already wired through `factory.py`/`model_catalog.py`/`cli/utils.py`, and #1146 was already satisfied by `.env.example` since the pr-1039 merge. The #1123-prompted divide-by-zero audit cleared: every ratio in `evaluation/benchmark.py` is guarded. Upstream has still merged nothing since #579 (2026-04-25).

- **Dashboard bind/auth/CSP hardening** (prompted by upstream #1160's audit item, `docs/UPSTREAM_PR_INTEGRATION_PLAN.md` §3.2): `dashboard/app.py` previously bound `0.0.0.0:8050` unconditionally with no authentication, CSP, or host allowlist, exposing portfolio/PnL data to the network by default. Adds an optional `TRADINGAGENTS_DASHBOARD_TOKEN` shared-secret gate — checked via the `X-Dashboard-Token` header only, on every request, through a Flask `before_request` hook, compared with `hmac.compare_digest` (an earlier revision also accepted `?token=` in the query string; dropped in PR review since a token in the URL persists in browser history, reverse-proxy/access logs, and Referer headers) — and a `Content-Security-Policy`/`X-Frame-Options`/`X-Content-Type-Options` response-header set via `after_request`. The `if __name__ == "__main__":` entry point refuses to start when bound to a non-loopback host without a token configured — but that guard only covers `python dashboard/app.py` directly, never a WSGI deployment (gunicorn/fly.io) that imports `server = app.server` and skips `__main__` entirely, so the real gate is enforced per-request instead: with no token configured, `_require_dashboard_token` now rejects any request whose `request.remote_addr` isn't loopback (also caught in PR review — the original version only checked "is a token supplied," which was a no-op when no token existed at all). (`dashboard/app.py`, `tests/test_dashboard_security.py`)

- **Machine-readable run manifest** (ports upstream #1179, `docs/UPSTREAM_PR_INTEGRATION_PLAN.md` §3.1): `reports/exporter.py::save_report_to_disk` now writes `run_manifest.json` next to `complete_report.md` whenever both `selections` and `config` are provided (same gating `run_config.md` already uses) and the new `run_manifest_enabled` config key is true (default). New `tradingagents/reports/manifest.py::build_run_manifest()` records the requested as-of date, asset type, selected analysts, provider/model IDs, temperature, a sanitized backend URL (credentials/query/fragment stripped), configured vendor chains, debate/risk-round limits, a config-fingerprint hash, per-section content hashes for every analyst/debate/risk report present, the parsed final rating, and the final decision's output hash — built on this fork's own audit primitives (`audit/schemas.py::canonical_json` for hashing, `audit/prompt_registry.py` for per-template SHA-256 digests of every configured `prompt_versions` entry) rather than upstream's standalone re-implementation, and targeting this fork's actual `reports/exporter.py` writer rather than upstream's `reporting.py` (a same-named module this fork also has, serving a different, simpler writer used only by `TradingAgentsGraph.save_reports()` — not touched here; wiring the manifest into that second path is a follow-up, not a gap in this change). Only a curated subset of config is captured by construction, so local absolute paths (`results_dir`, `data_cache_dir`, `project_dir`, ...) never appear in the file. Records *configured* vendor-chain and prompt identity, not which fallback vendor actually served a given call — documented as "auditable and comparable," not "reproducible," per the honest scope boundary upstream's own author raised. The manifest build+write is wrapped in `try/except` (caught in PR review — `complete_report.md`/`run_config.md` are already on disk by the time the manifest builds, so a hashing or serialization failure in this additive extra must not make `save_report_to_disk` look like it failed outright); `json.dumps(..., default=str)` also guards against a non-JSON-serializable value reaching the file. (`tradingagents/reports/manifest.py`, `tradingagents/reports/exporter.py`, `tradingagents/default_config.py`, `tradingagents/config_schema.py`, `docs/CONFIG_REFERENCE.md`, `tests/test_run_manifest.py`)

### Changed

- **Dashboard default bind host changed to loopback**: `dashboard/app.py`'s dev entry point now binds `127.0.0.1` by default instead of `0.0.0.0`, overridable via the new `TRADINGAGENTS_DASHBOARD_HOST` env var. Anyone currently reaching the dashboard from another machine on `0.0.0.0` needs to set `TRADINGAGENTS_DASHBOARD_HOST=0.0.0.0` explicitly **and** `TRADINGAGENTS_DASHBOARD_TOKEN` for the process to start — see the "Added" entry above. (`dashboard/app.py`)
...
- **Snapshot fundamentals leaking future data into historical/backtest runs (upstream PR review, workstream 2)**: `y_finance.py::get_fundamentals` and `alpha_vantage_fundamentals.py::get_fundamentals` both only *prepended a warning string* (`historical_snapshot_caveat`) when `curr_date` was in the past — the actual snapshot-only, price-derived fields (market cap, P/E and other valuation ratios, dividend yield, Beta, 52-week high/low, 50/200-day moving averages) still leaked into the report unchanged, since they only ever reflect *today's* price regardless of what date was requested. New `point_in_time.py::is_historical_date()` (refactored out of `historical_snapshot_caveat`) gates a new field-stripping step in both vendor functions — `y_finance.py`'s `_SNAPSHOT_ONLY_LABELS` and `alpha_vantage_fundamentals.py`'s `_SNAPSHOT_ONLY_KEYS` name the affected fields per vendor's schema. The remaining financial-statement-derived fields (revenue, EPS, margins, ROE, etc.) are **not** point-in-time either — they still reflect the company's most recently reported quarter as of *today*, not as of `curr_date`, just on a slower (quarterly) update cadence than the price-derived fields — the caveat text now says so explicitly and points to `get_balance_sheet`/`get_cashflow`/`get_income_statement`, which do filter reports by date, for anything that needs a real historical figure. Updated `test_fundamental_lookahead.py`'s existing test, which had asserted the old (buggy) pass-through behavior. Ports upstream #1163. (`tradingagents/dataflows/point_in_time.py`, `y_finance.py`, `alpha_vantage_fundamentals.py`, `tests/test_fundamentals_snapshot_leak.py`, `tests/test_fundamental_lookahead.py`)
