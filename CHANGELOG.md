...
- **Dashboard bind/auth/CSP hardening** (prompted by upstream #1160's audit item, `docs/UPSTREAM_PR_INTEGRATION_PLAN.md` §3.2): `dashboard/app.py` previously bound `0.0.0.0:8050` unconditionally with no authentication, CSP, or host allowlist, exposing portfolio/PnL data to the network by default. Adds an optional `TRADINGAGENTS_DASHBOARD_TOKEN` shared-secret gate (checked via `?token=` query param or `X-Dashboard-Token` header on every request through a Flask `before_request` hook) and a `Content-Security-Policy`/`X-Frame-Options`/`X-Content-Type-Options` response-header set via `after_request`. The `if __name__ == "__main__":` entry point now refuses to start when bound to a non-loopback host without a token configured, raising a clear error instead of silently serving to the network. (`dashboard/app.py`, `tests/test_dashboard_security.py`)
