...
def get_ticker(default_ticker: str = "SPY") -> str:
    ...
    return normalize_ticker_symbol(ticker) if ticker.strip() else default_ticker
...
def select_openrouter_model(mode: str) -> str:
    ...
    """
    models = _prefer_openrouter_free_models(_fetch_openrouter_models())  # free first, newest first
    ...
    choice = questionary.select(
        f"Select Your [{mode.title()}-Thinking] OpenRouter Model (latest available, free first):",
        choices=choices,
        instruction="\n- Use arrow keys to navigate\n- Press Enter to select",
        style=questionary.Style([
