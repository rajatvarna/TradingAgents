...
**Status:** All three phases complete. F5/F6 landed under
`docs/PROMPT_AND_CORE_FEATURES_PLAN.md`'s workstream B rather than this plan's
own originally-numbered branches (see §6). F6's originally-planned
`ops/exits`/`ops/position_guardian` consumer wiring was investigated and
retired as unnecessary — both already operate at the cadence appropriate to
their role (see §6 for why).
...
### F5. Typed, validated configuration layer — IMPLEMENTED (as B1)
...
### F6. Intraday data-interval support — DATA LAYER IMPLEMENTED (as B2); ops consumer wiring retired, see §6
...
```text
Phase 0 — ALL DONE               Phase 1                   Phase 2 — ALL DONE
─────────────────────            ───────────────────       ──────────────────
F2 alpaca broker  ✅ done         F5 typed config  ✅ done   F6 intraday data ✅ done
F2 ibkr broker    ✅ done         F4 scanner loop  ✅ done      (ops consumer wiring
F3 budget cap     ✅ done                                       retired, not needed)
...
- **F5 shipped** as `tradingagents/config_schema.py::TradingAgentsConfig` (landed
  under the B1 workstream in `docs/PROMPT_AND_CORE_FEATURES_PLAN.md`, same
  design this section specified: additive pydantic model, warn-only
  `validate_config()` wired into `TradingAgentsGraph.__init__`, strict mode via
  `strict_config`/`TRADINGAGENTS_STRICT_CONFIG`, autogenerated
  `docs/CONFIG_REFERENCE.md`, `python -m tradingagents.config_schema
  {check,generate-docs}`).
- **F6 shipped as the dataflows-layer fetcher; the ops/exits consumer wiring
  this section originally called for turned out not to be needed.**
  `dataflows/interface.py::get_intraday_stock_data()` landed under the B2
  workstream in `docs/PROMPT_AND_CORE_FEATURES_PLAN.md`: a new, separate
  vendor-router method (Alpha Vantage `TIME_SERIES_INTRADAY` + Polygon
  aggregates) with `VendorCapabilityError` fallthrough and a TTL-aware
  cache, exactly as designed below. Wiring it into `ops/exits/engine.py`
  and `ops/position_guardian.py` was investigated and found unnecessary once
  the actual live-trading code was read closely (this plan's own opening
  revision note flagged the same failure mode for F1-F4 — the original
  problem statement underestimated how mature `ops/` already is):
  - `ops/position_guardian.py` never called yfinance's daily-history endpoint
    to begin with — its `quote_source` is a live point quote (`ops/quotes.py`),
    polled every 60 seconds by the scheduler (`ops/main.py`). That's already
    more timely than any interval-bar fetch (even 1-minute bars) would be;
    swapping in `get_intraday_stock_data`'s 15-minute-TTL cache would make
    stop-loss checks *less* responsive, not more.
  - `ops/exits/engine.py`'s `trend_break` rule requires **two consecutive
    daily closes** below the 200-day SMA before firing — explicit,
    documented hysteresis ("a single close must not trigger an exit
    (hysteresis / whipsaw)"). A 200-day trend filter reacting to intraday
    bars would defeat that whipsaw protection by design, not improve exit
    quality. The engine's other two rules (`rank_decay`, `max_hold`) aren't
    price-fetch-driven at all.
  No consumer in `ops/` has an actual use for interval data narrower than a
  daily close; the fetcher exists in `dataflows/` for a future consumer that
  needs it; `exit_check_interval` was never added since there's nothing for
  it to gate yet.
- **Phase 0, Phase 1, and Phase 2 are complete.** F6 shipped its data-layer
  half; the "M3 intraday-aware exits" milestone is retired rather than
  pending — see the F6 note above for why.
- **Milestone M1 (paper-traded, budget-capped, unattended single-cycle
  flow): reached** — F2, F3, F4 are all in place.
- **Milestone M2 (validated config): reached** — F5 done.
- **Milestone M3 (intraday-aware exits): retired, not applicable** — both
  exit-decision paths already operate at the cadence appropriate to their
  role (60s live quotes for hard stops; two-daily-close hysteresis for the
  200-day trend filter). See the F6 note above.
...
`claude/core-features-plan-gwdmiq` per this session's instructions; F5/F6 landed
under the prompt-improvements branch/PR instead (`docs/PROMPT_AND_CORE_FEATURES_PLAN.md`
workstream B), since that program absorbed both open core features per its own
scope note.
