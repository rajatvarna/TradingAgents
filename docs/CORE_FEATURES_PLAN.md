...
**Status:** Phase 0 and Phase 1 complete; Phase 2 (F6 intraday data) partially
done — the dataflows-layer fetcher landed (see §6), but `ops/exits/engine.py`
and `ops/position_guardian.py` don't consume it yet. F5/F6 landed under
`docs/PROMPT_AND_CORE_FEATURES_PLAN.md`'s workstream B rather than this plan's
own originally-numbered branches (see §6).
...
### F6. Intraday data-interval support — PARTIALLY IMPLEMENTED (as B2)
...
─────────────────────            ───────────────────       ──────────────────
F2 alpaca broker  ✅ done         F5 typed config  ✅ done   F6 intraday data
F2 ibkr broker    ✅ done         F4 scanner loop  ✅ done      🟡 dataflows layer only
F3 budget cap     ✅ done
...
- **F5 shipped** as `tradingagents/config_schema.py::TradingAgentsConfig` (landed
  ...
  {check,generate-docs}`).
- **F6 partially shipped** as `dataflows/interface.py::get_intraday_stock_data()`
  (landed under the B2 workstream in `docs/PROMPT_AND_CORE_FEATURES_PLAN.md`):
  a new, separate vendor-router method (Alpha Vantage `TIME_SERIES_INTRADAY` +
  Polygon aggregates) with `VendorCapabilityError` fallthrough and a
  TTL-aware cache, exactly as designed below. **Not yet done:** the consumer
  half — `ops/exits/engine.py` and `ops/position_guardian.py` still call
  yfinance directly and evaluate stops against daily closes only; wiring in
  `exit_check_interval` (default `"1d"`, no behavior change until opted in) is
  the remaining piece of this feature.
- **Phase 0 and Phase 1 are complete.** Phase 2 (F6) is partially done —
  dataflows-layer intraday fetching exists; the ops/exits consumer wiring is
  the one remaining gap in this whole plan.
...
- **Milestone M3 (intraday-aware exits): not yet reached** — F6's data layer is
  done, but `ops/exits/engine.py`/`ops/position_guardian.py` don't consume it
  yet (see F6 above).
