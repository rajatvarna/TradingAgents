...
| D-2 | **Portfolio & journal view** (`/portfolio`) | Rescoped from a new `GET /portfolio` + `GET /journal?since=` pair to reusing `ops.status.build_status()` directly — it's the ops CLI's own journal-only status snapshot, already documented as safe to call alongside the live service (WAL concurrent reads, no broker/network calls). `api/ops_view.py::get_portfolio_status()` gates the whole feature behind `OPS_JOURNAL_PATH`: unset means the ops daemon isn't assumed to be on this host and the endpoint returns 503, never a guess. `GET /portfolio` (bearer-auth protected, like `/env` — it exposes real positions/cash) returns the JSON-safe snapshot; still strictly read-only, no order-placement surface added anywhere. | 3 unit tests (`tests/test_api_ops_view.py`), mirroring `tests/ops/test_cli_status.py`'s fixture. | **Done** |
...
**Why D-3/D-4 are not started, and why D-2 turned out safe to do anyway:**
all three reach into `ops/` — the separate, already-running live-trading
daemon (guarded broker, event-sourced journal, scheduled scanner loop) —
which is a fundamentally different trust and risk domain than the
analysis-job API and screener frontend this plan otherwise touches.
Investigating before assuming that meant something different for each:

- **D-2** turned out low-risk on inspection: `ops/status.py::build_status()`
  already exists, is read-only by construction (its own docstring guarantees
  no broker/network calls), and is explicitly documented as safe to run
  alongside the live daemon — the ops CLI's `ops status` command already
  does exactly this. The only real design question (same-host vs.
  cross-host) was resolved by gating on `OPS_JOURNAL_PATH` rather than
  guessing, so it shipped in this pass.
- **D-3** would change what tickers the live scanner evaluates — a
  ...
  Read-only visibility (D-2) and changing scanner input (D-3) are not
  the same risk tier even though both touch `ops/`.
- **D-4** is lower risk than D-3 but depends on `notify.py`'s existing
  channel contracts, which were not re-verified in this pass; scope it
  once someone confirms what channels are actually configured in
  production.

Recommended: scope D-3 and D-4 as their own follow-ups, starting with a short
design note (mirroring this document's format) once there's a chance to
review the current `ops/` scanner and `notify.py` config with whoever
operates the daemon.
...
| 4 | M per feature | D-2 exposes trading state over HTTP | Mitigated: bearer auth from Phase 1, `OPS_JOURNAL_PATH` opt-in gating, reuses the already-safe `ops.status.build_status()`, no order mutation in the web tier |
