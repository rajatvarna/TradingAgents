...
- **Cost guards**: every guard-bearing function ships with an `enabled: bool = False` default. Measurement is unconditional; enforcement is gated. (See saved memory `cost-guards-disabled-by-default.md`, local-only.)
