...
| **R-F2-4** | **stockstats vwma/macdh `Date` KeyError** (known, see saved memory `stockstats-vwma-macdh-date-error.md`, local-only) surfaces during 15 graph runs | Low | Non-blocking — those two indicators silently fail and the analyst proceeds. **Defer fix beyond F2**, document in exit-gate report header. |
