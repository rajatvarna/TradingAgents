...
export async function POST(req: NextRequest) {
  try {
    ...
    const quoteMap = new Map<string, { price: number; currency: string; marketCap: number; volume: number; avgVolume: number; fiftyTwoWeekHigh: number | null; fiftyTwoWeekLow: number | null }>();
      ...
      const batchResults = await Promise.all(
        batch.map((t) => {
          ...
          return resolveStock(
            ...
            q?.fiftyTwoWeekHigh ?? undefined, q?.fiftyTwoWeekLow ?? undefined
          );
        })
      );
