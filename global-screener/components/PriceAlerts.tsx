...
export default function PriceAlerts({ stocks }: Props) {
  ...
  // Check alerts whenever the screener's stock data updates (no extra API calls)
  useEffect(() => {
  ...
  const addAlert = () => {
    ...
    const isTracked = stocks.some((s) => s.symbol.toUpperCase() === sym);
    const alert: Alert = { id: uid(), symbol: sym, price, direction: dirInput, untrackable: !isTracked };
                ...
                {alerts.map((a) => (
                  ...
                  >
                    <span className="text-white font-semibold">
                      {a.symbol}
                      {a.untrackable && (
                        <span className="ml-1 text-[10px] text-amber-500" title="Symbol not in watchlist — alert may not trigger">⚠</span>
                      )}
                    </span>
