...
function applyFilters(data: StockData[], f: FilterState, starred: Set<string>): StockData[] {
  ...
  // 52w-highs preset filter: within 5% of 52-week high
  if (f.search === "__52w-highs__" || f.only52wHigh) {
    rows = rows.filter((d) => d.fiftyTwoWeekHighChangePct !== null && d.fiftyTwoWeekHighChangePct >= -5);
  ...
  if (f.onlyStarred) {
    rows = rows.filter((d) => starred.has(d.symbol));
  }
...
export default function ScreenerTable({ onSelectStock, onDataLoaded }: Props) {
  ...
  function toggleStar(symbol: string, e: React.MouseEvent) {
    e.stopPropagation();
    setStarred((prev) => {
      ...
      try {
        localStorage.setItem("starred", JSON.stringify(Array.from(next)));
      } catch {
        // ignore
      }
      return next;
  ...
  const exportCSV = useCallback(() => {
    ...
    const lines = rows.map((r, i) => [
      ...
      r.fiftyTwoWeekHigh ?? "N/A",
      r.fiftyTwoWeekLow ?? "N/A",
      r.fiftyTwoWeekHighChangePct?.toFixed(2) ?? "N/A",
      r.marketCap ?? "N/A",
      r.volume ?? "N/A",
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:hidden">
        {isLoading
          ? Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="rounded-xl border border-slate-700 bg-slate-900 p-3 animate-pulse space-y-2">
          ...
          : rows.map((stock) => {
              ...
              return (
                <div
                  key={stock.symbol}
                  onClick={() => onSelectStock(stock)}
                  className="rounded-xl border border-slate-700 bg-slate-900 p-3 cursor-pointer hover:bg-slate-800/60 transition-colors"
                >
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-white text-sm">{stock.symbol}</span>
                      <span className="text-xs text-slate-500">{MARKET_FLAG[stock.market]} {stock.market}</span>
                    </div>
                    <button
                      onClick={(e) => toggleStar(stock.symbol, e)}
                      className="text-sm hover:scale-125 transition-transform"
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-white font-mono text-sm">
                      {stock.price !== null ? stock.price.toLocaleString(undefined, { maximumFractionDigits: 4 }) : "N/A"}
                      <span className="text-slate-600 text-xs ml-1">{stock.currency}</span>
                    </span>
                    <span className={cn("font-mono text-sm", pctColor(stock.performance.daily))}>
                      {fmtPct(stock.performance.daily)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between mt-1 text-xs">
                    <span className="text-slate-500">YTD: <span className={pctColor(stock.performance.ytd)}>{fmtPct(stock.performance.ytd)}</span></span>
                    <span className="text-slate-500">Cap: {fmtMarketCap(stock.marketCap)}</span>
                  </div>
                </div>
              );
            })}
