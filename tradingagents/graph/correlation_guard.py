...
_MIN_CORRELATION_SAMPLES = 20  # minimum data points required for correlation
...
def max_pairwise_correlation(
    ...
    if new_returns is None or len(new_returns) < _MIN_CORRELATION_SAMPLES:
        return None
    ...
    for ticker in existing_tickers:
        ...
        if existing_returns is None or len(existing_returns) < _MIN_CORRELATION_SAMPLES:
            continue
        try:
            aligned = pd.concat(
                [new_returns, existing_returns], axis=1, keys=["new", "existing"]
            ).dropna()
            if len(aligned) < _MIN_CORRELATION_SAMPLES:
                continue
            corr = abs(float(aligned["new"].corr(aligned["existing"])))
            if corr > max_corr:
                max_corr = corr
        except Exception:
            continue
...
def apply_correlation_sizing_adjustment(
    ...
    if max_corr is None:
        return position_size_pct, "Correlation check skipped (insufficient data)."
